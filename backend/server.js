...
app.use(
  cors({
    origin: ["http://localhost:5173", "https://fastmarketplace.netlify.app"],
    credentials: true,
  })
);
...
morgan.token("user", (req) =>
  req.user ? `user:${req.user.id}` : "user:anonymous"
);
...
const customFormat =
  ":date :remote-addr :method :url :status - :response-time ms :user - agent::user-agent body::body";
app.use(
  morgan(customFormat, {
...
app.use((err, req, res, next) => {
  const errorLog = `${new Date().toISOString()} - ❌ Error en ${req.method} ${
    req.originalUrl
  } - ${err.message} - Body: ${JSON.stringify(req.body)}\n`;
...
if (require.main === module) {
  pool
    ...
    .catch((err) => {
      const errorMsg = `${new Date().toISOString()} - ❌ Error al conectar con la base de datos: ${
        err.message
      }\n`;
